...
//dd(($importData_arr[2][10]));
        ...
        //legeal checker offense fex
        if ($request->hasFile('legealcheckeroffensefex')) {
            $file = $request->file('legealcheckeroffensefex');
            ...
            if (in_array(strtolower($extension), $valid_extension)) {
                ...
                if ($fileSize <= $maxFileSize) {
                    ...
                    $importData_arr = array();
                    $i = 0;
                    $key_word = legealCheckerFexOffense::truncate();
                    $i = 0;
                    while (($filedata = fgetcsv($file, 1000, ",")) !== FALSE) {
                        $num = count($filedata);
                    ...
                    foreach ($importData_arr as $importData) {
                        ...
                        if ($importData[0] != "") {
                            $keyword = new legealCheckerFexOffense();
                            $keyword->offense_id = intval($importData[0]);
                            $keyword->offense_e = utf8_decode($importData[1]);
                            $keyword->offense_s = utf8_decode(($importData[2]));
                            $keyword->company = utf8_decode(($importData[3]));
